class LCDmenu:
    ...
    def levelDescent(self):
        if self.AllItems.get(self.CurrentItem + ".0") is not None:
    ...
    def getNextItem(self):
        ...
        if self.AllItems.get(newitem) is not None:
            return(newitem)
        else:
            if self.CurrentLevel > 0:
                return(levelBase + ".0")
    ...
    def getPrevItem(self):
        ...
        if self.AllItems.get(newitem) is not None:
            ...
            if self.CurrentLevel > 0:
                return(levelBase + "."
                       + str(len(self.getLevelItemList()) - 1))
            else:
    ...
    def currentItemIsAction(self):
        if self.AllItems.get(self.CurrentItem + ".0") is None:
            return(True)
        else:
            return(False)
